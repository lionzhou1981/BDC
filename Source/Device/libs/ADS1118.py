...
def list_to_bytearray(data):
    return bytearray(
        list_to_int(data[i:i + 8]) for i in range(0, len(data), 8))
...
gains = {  # calculate the value of the least significant bit (LSB) by dividing the full range by the number of available bits
    0:
    6.144 * 2 / 2**16,  # 187.5 microvolts per bit at +- 6.144 volts full range
...
def decode(data):
    ...
    return dict(single_shot=bool(data[0]),
...
INT_TEMP = encode(single_shot=True, temp_sensor=True,
                  data_rate=5)  # read internal temperature
...
class ADS1118(object):
    ...
    def __init__(self,
        ...
        time.sleep(
            .030
        )  # something sets high, and the ADS1118 needs to have the clock held low for 28 ms to reset (datasheet page 22, section 9.5.3)
    ...
    def read(self, *commands):
        ...
        for command in commands:
            ...
            if self.pause_mode:
                ...
                self.wait()
        responses.append(
            self._read(INT_TEMP))  # dummy command to get the last data value

        configs, datas = zip(*responses)
        results = []
        for command, config, data in zip(
                commands, configs,
                datas[1:]):  # skip the first data since it's residual
